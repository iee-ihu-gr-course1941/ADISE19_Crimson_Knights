...
function play_special_card() {
    ...
    $('#top_card').empty();
    draw_card(special_card_id[0], special_card_id[1], special_card_id[2], special_card_id[3], '#top_card');
    json_data = JSON.stringify({
        USER_TOKEN: me.token,
        card: {
...
function sign_in() {
    ...
    if ($('#sign_in').val().toLocaleLowerCase() == 'na' || $('#sign_in').val().toLocaleLowerCase() == 'top_card') {
        alert("You can't sign in as that user");
        $('#sign_in').val('');
        return;
    }
    json_data = JSON.stringify({
...
function sign_up() {
    ...
    if ($('#sign_up').val().toLocaleLowerCase() == 'na' || $('#sign_up').val().toLocaleLowerCase() == 'top_card') {
        alert("You can't sign up as that user");
        $('#sign_up').val('');
        return;
    }
    json_data = JSON.stringify({
...
function get_initial_board_state() {
    ajax_request('https://users.iee.ihu.gr/~it174973/uno.php/get_initial_board_state', 'GET', 'check_initial_board_state');
}

function set_board_state(data){
    $('#game_state h3').html(data[0].BOARD_STATE);
}

function get_board_state() {
    ajax_request('https://users.iee.ihu.gr/~it174973/uno.php/get_board_state', 'GET', 'set_board_state');
}
